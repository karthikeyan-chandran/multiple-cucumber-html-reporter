...
=====================================================================================
    Mobile Sales Tool Cucumber HTML report generated in:
...
=====================================================================================\n`));
  ...
  // const example = async () => {
  //   console.log(chalk.blue('About to snooze without halting the event loop...'));
  //   await snooze(5000);
  ...
  //     //const templatePath = Path.resolve(`./report`, 'index.html')
  //     const templatePath = Path.resolve(reportPath, 'index.html')
  //     const imagePath = Path.resolve(reportPath, 'MST_Report.png')
  //     const pdfPath = Path.resolve(reportPath, 'MST_Report.pdf')
  ...
  // })();
  function _printPDF() {
    console.log(chalk.blue("Print PDF"));
    const templatePath = path.resolve(reportPath, 'index.html')
    const imagePath = path.resolve(reportPath, 'MST_Report.png')
    const pdfPath = path.resolve(reportPath, 'MST_Report.pdf')
    puppeteer.launch().then(browser => {
      ...
      browser.newPage()
        .then(page => {
          ...
          page.goto("file://" + templatePath, { waitUntil: 'networkidle2' })
            .then(resp => {
    ...
    //     page.then(pg => {
    //         pg.goto("file://" + templatePath).then(() => {
    //             console.log(chalk.blue("file loaded browser"));
    //             pg.screenshot({
    //                 path: imagePath
    //             }).then(() => {
    //                 brw.close();
    ...
    console.log(chalk.blue("Print PDF2"));
  }
